...
class Pinjaman extends CI_Controller {
	...
	//controller untuk menampilkan semua pinjaman
	public function index()
	{
		$data['pinjaman'] = $this->model_pinjaman->semua_pinjaman();
		$this->load->view('admin/pinjaman_index', $data);
	}

	//controller untuk menampilkan detail pinjaman
	public function lihat($id_pinjaman)
	{
		$data['pinjaman'] = $this->model_pinjaman->find_id($id_pinjaman);
		$this->load->view('admin/pinjaman_detail', $data);
	}

	//controller untuk tambah pinjaman
	public function tambah()
	{	
		$this->form_validation->set_rules('id_users', 'Anggota', 'required');
		$this->form_validation->set_rules('id_kwarran', 'Kwarran', 'required');
		$this->form_validation->set_rules('jml_pinjaman', 'Besar Pinjaman', 'required');
		$this->form_validation->set_rules('bsr_bunga', 'BUnga', 'required');
		$this->form_validation->set_rules('lama_angsuran', 'Lama Angsuran', 'required');
		$this->form_validation->set_rules('tgl', 'Tanggal', 'required|trim|xss_clean');
		if ($this->form_validation->run() == FALSE)
		{
			$data['propinsi'] = $this->db->get('kwarran');
			$this->load->view('admin/pinjaman_tambah', $data);	
		}else{
			$data_pinjaman = array(
					'id_kwarran'	=> set_value('id_kwarran'),
					'id_users'		=> set_value('id_users'),
					'jml_pinjaman'	=> set_value('jml_pinjaman'),
					'bsr_bunga'		=> set_value('bsr_bunga'),
					'lama_angsuran'	=> set_value('lama_angsuran'),
					'tgl'			=> set_value('tgl'),
					'status'		=> 'Belum Lunas'
				);
				$this->model_pinjaman->tambah_pinjaman($data_pinjaman);
	...
	{
		$this->form_validation->set_rules('id_users', 'Anggota', 'callback_ada_user');
		$this->form_validation->set_rules('id_kwarran', 'Kwarran', 'required');
	...
	//controller untuk menghapus pinjaman
	public function hapus($id_pinjaman)
	{
		$this->model_pinjaman->hapus($id_pinjaman);
	...
	function kabupaten(){
        ...
        {
            echo "<option value='$k->id_users'>$k->nama_users</option>";
        }
        echo "</select></div>";
    }

   function ada_user($id_users){
    $this->model_pinjaman->user_ada($id_users);
	}
